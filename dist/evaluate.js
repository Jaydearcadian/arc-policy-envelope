...
const parseAmount = (value, field, allowZero = false) => {
    if (!DECIMAL_6.test(value))
        return `${field} must be a positive decimal string with at most 6 decimals`;
...
const isWithinSchedule = (envelope, requestedAt) => {
    const date = new Date(requestedAt);
    if (Number.isNaN(date.getTime()))
        return false;
    const day = WEEKDAYS[date.getUTCDay()];
    const hour = date.getUTCHours();
    if (!envelope.schedule.days.includes(day))
        return false;
...
const evaluateReasons = (envelope, request) => {
    const reasons = [];
    ...
    if (typeof spent === "string")
        reasons.push(spent);
    if (typeof maxPerAction === "string")
        reasons.push(maxPerAction);
    if (typeof maxPerPeriod === "string")
        reasons.push(maxPerPeriod);
    if (request.network !== envelope.asset.network)
        reasons.push(`network ${request.network} is not allowed`);
    if (request.asset !== envelope.asset.asset)
        reasons.push(`asset ${request.asset} is not allowed`);
...
export const evaluateEnvelope = (envelope, request, evaluatedAt) => {
