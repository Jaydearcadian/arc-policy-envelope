...
const bodyEnvelope = (body) => {
    if (body && typeof body === "object" && "envelope" in body)
...
const bodyEvaluation = (body) => {
    if (body && typeof body === "object" && "evaluation" in body)
        return body.evaluation;
    return body;
};
const handlePost = async (path, request) => {
    const body = await readJson(request);
    if (path === "/envelopes/hash")
        return { status: 200, body: { policyHash: hashEnvelope(bodyEnvelope(body)) } };
    if (path === "/requests/hash")
        return { status: 200, body: { requestHash: hashRequest(bodyRequest(body)) } };
    if (path === "/evaluations/hash")
        return { status: 200, body: { evaluationHash: hashEvaluation(bodyEvaluation(body)) } };
